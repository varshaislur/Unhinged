...
export const updateUser = async (req, res, next) => {
  try {
    const {firstName , lastName, contact , profession , location} = req.body

    if(!(firstName || lastName || contact || profession || location)){
        next("no field can be left empty");
        return;
    }
...
export const getFriendRequest = async (req, res, next) =>{
  try{
    ...
    }).limit(10).sort({_id:-1})//descending order sort, 10 people
...
export const profileViews=async(req,res,next)=>{
  ...
  const user =await Users.findById(id)//user whose profile is viewed
