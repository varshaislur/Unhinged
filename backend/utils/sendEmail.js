...
export const sendVerification = async (user, res) => {
    ...
    <h5>Stay unhinged, stay awesome</h5>
        <h5>Best Regards</h5>
        <h5>Unhinged Team</h5>
        </div>
    </div>`,
    ...
    try{
        ...
        if(newVerifiedEmail){
            transporter.sendMail(mailOptions).then(()=>{
                res.status(201).send({
                    success:"PENDING",
                    message:"verification email has been sent to your email. check that!!!!!"
                })
            }).catch((error)=>{
                console.log(error);
                res.status(404).json({message: "something went wrong while sending email"})
